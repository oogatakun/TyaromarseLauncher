    ...
    for(let ele of document.getElementsByClassName('settingsFileSelButton')){
        ...
        ele.onclick = async e => {
            ...
            if(isJavaExecSel && process.platform === 'win32') {
                options.filters = [
                    { name: Lang.queryJS('実行可能ファイル'), extensions: ['exe'] },
                    { name: Lang.queryJS('すべてのファイル'), extensions: ['*'] }
                ]
            }
        ...
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    ...
                    'チャロマーズランチャーを使うためには、最低1個のアカウントにログインしている必要があります。<br>ログアウトしたら、もう一度ログインしないといけなくなります。<br><br>本当にログアウトする？',
                    '分かってるよそんなこと',
                    'やっぱやめとく'
                )
                setOverlayHandler(() => {
